...
//! - `remove_files`: paths of surviving Remove actions in the range.
//!
//! To advance a delta-on-base file listing cache, append `add_files` to the delta layer and
//! union `remove_files` with `duplicate_add_paths` to mask base entries. Both sets are
//! required: `remove_files` masks paths that left the table; `duplicate_add_paths` masks the
//! stale base entry of each path that the range re-added with new metadata.
//!
//! V2 checkpoint sidecars and log compaction files in the range are not consulted; commit
//! JSONs are read directly. Both are optimizations over the same underlying actions.
//!
//! [`IncrementalScanResult::CommitsUnavailable`] is returned upfront when the snapshot's
//! commit list starts past `base_version` (loaded from a checkpoint at version
//! `c >= base_version + 1`, or older JSONs were vacuumed away). Consumers fall back to a
...
//!         // mask = listing.remove_files U listing.duplicate_add_paths (set union)
//!     }
...
/// [`Operation::Scan`]. By the protocol's feature-immutability rule (features cannot be
/// removed once added), any reader feature present in an intermediate commit is also
/// present in the target snapshot's protocol; checking the target alone is sufficient.
...
impl IncrementalScanBuilder {
    ...
    ///
    /// # Errors
    /// Returns `Err` for caller errors (`base_version >= snapshot.version()`) or for
    /// unsupported reader features in the target snapshot's protocol.
    ///
    /// # Returns
    /// [`IncrementalScanResult::Listing`] on success, or
    /// [`IncrementalScanResult::CommitsUnavailable`] when the snapshot's commit list cannot
    /// reach `base_version + 1` (loaded from a checkpoint at version
    /// `c >= base_version + 1`, or the snapshot's earliest JSON was vacuumed). Also returned
    /// when a commit file the snapshot listed is missing at read time (vacuum race).
    pub fn build<P: AsRef<str>>(
        self,
        engine: &dyn Engine,
        base_file_paths: impl IntoIterator<Item = P>,
    ) -> DeltaResult<IncrementalScanResult> {
        // TODO(#2493): the version validation, the snapshot-commit-list extraction, the
        // "snapshot covers the range" check, and the per-commit read loop's
        // FileNotFound -> CommitsUnavailable mapping all collapse into a single
        // `CommitRangeBuilder::from_snapshot(...).build(engine)` call once that primitive
        // lands. https://github.com/delta-io/delta-kernel-rs/issues/2493
        ...
        // Confirm kernel supports the target's reader features. Mirrors `Scan::new`.
        self.snapshot
            .table_configuration()
        ...
        let commit_files = snapshot_commits
            .iter()
            .filter(|c| c.version >= start_version)
            .rev();
...
pub enum IncrementalScanResult {
    ...
    /// The snapshot's commit list cannot serve `(base_version, target_version]`.
...
/// file listing cache.
///
/// Apply via: append `add_files` to the delta layer; mask base entries using the union
/// `remove_files U duplicate_add_paths`. Both sets are required for correctness; see the
/// module-level docs.
#[non_exhaustive]
pub struct IncrementalListing {
    ...
    /// commit batch that produced any surviving Adds, in descending commit-version order
    /// (newest first). Includes both "truly new" adds and metadata-only re-adds; consumers
    /// do not need to distinguish when applying to a delta layer.
    pub add_files: Vec<FilteredEngineData>,
    /// Paths from `add_files` that also exist in the consumer's base file listing
    /// (metadata-only re-adds, e.g. OPTIMIZE / liquid clustering re-tag). Surfaced
    /// separately so consumers can mask matching base entries or mirror this set for
    /// telemetry. The corresponding rows are still present in `add_files`.
    pub duplicate_add_paths: HashSet<String>,
    /// Paths of surviving Remove actions in the range. Consumers must union this with
    /// `duplicate_add_paths` when masking the base; the union is what makes metadata-only
    /// re-adds correct (the new Add is the live entry, the stale base copy must be masked).
