...
impl LogSegment {
    ...
    /// - JSON checkpoint (always V2): extract sidecars, read first sidecar's schema
    /// - Parquet checkpoint: read checkpoint schema from hint or footer
    ///   - Has sidecar column (V2): extract sidecars, read first sidecar's schema
    ///   - No sidecar column (V1, including multi-part): use checkpoint schema directly
    fn get_file_actions_schema_and_sidecars(
        &self,
        engine: &dyn Engine,
    ) -> DeltaResult<(Option<SchemaRef>, Vec<FileMeta>)> {
        // Use the first checkpoint part for schema detection. Multi-part checkpoints share
        // an identical schema across all parts (they represent one logical checkpoint).
        ...
        match checkpoint.extension.as_str() {
            ...
            "parquet" => {
                // Resolve the checkpoint schema from hint or footer (single read).
