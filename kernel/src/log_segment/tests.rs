...
async fn test_create_checkpoint_stream_returns_checkpoint_batches_if_checkpoint_is_multi_part(
    ...
    // Multi-part checkpoints are always V1 (no sidecars). Verify that the reader returns
    // the checkpoint batches directly without sidecar processing.
    ...
    add_checkpoint_to_store(&store, add_batch_simple(v1_schema.clone()), checkpoint_part_1)
        .await?;
    ...
    let checkpoint_read_schema = get_commit_schema().project(&[ADD_NAME])?;
    ...
    // Both parts should be returned as checkpoint batches (not log batches)
    for _ in 0..2 {
