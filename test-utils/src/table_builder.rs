    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            LogState::CheckpointV1 {
                checkpoint_at,
                total_versions,
            } => write!(f, "checkpoint_v1(at={checkpoint_at}, total={total_versions})"),
            LogState::CheckpointV2 {
                checkpoint_at,
                total_versions,
            } => write!(f, "checkpoint_v2(at={checkpoint_at}, total={total_versions})"),
            LogState::CheckpointAndCommits {
                checkpoint_at,
                commits_after,
            } => write!(
...
/// Partition column configuration for cross-product testing.
///
...
impl PartitionConfig {
    ...
    pub fn columns(&self) -> Vec<&'static str> {
        match self {
            PartitionConfig::Unpartitioned => vec![],
            PartitionConfig::AllTypes => vec![
                "part_ts_ntz",
                "part_bool",
                "part_string",
                "part_date",
...
impl fmt::Display for PartitionConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PartitionConfig::Unpartitioned => write!(f, "unpartitioned"),
            PartitionConfig::AllTypes => write!(f, "partitioned(all_types)"),
        }
    }
}

/// Schema with all partition-valid primitive types. Use with [`PartitionConfig`] to select
...
impl TestTableBuilder {
    ...
    pub fn partition_columns(mut self, cols: impl IntoIterator<Item = impl Into<String>>) -> Self {
        self.partition_columns = cols.into_iter().map(Into::into).collect();
        self
    }

    /// Apply a [`PartitionConfig`], setting both the [`partitioned_schema`] and partition
    /// columns. For [`PartitionConfig::Unpartitioned`], leaves the schema and columns
    /// unchanged.
    pub fn partition_config(self, config: PartitionConfig) -> Self {
        let cols = config.columns();
        if cols.is_empty() {
            return self;
        }
        self.schema(partitioned_schema())
...
async fn write_data_commit(
    ...
    for file_idx in 0..num_files {
        ...
        let add_files = engine
            .write_parquet(&ArrowEngineData::new(batch), &write_context, partition_values)
            .await?;
...
mod tests {
    ...
    fn test_partitioned_table() -> DeltaResult<()> {
        let partition_config = PartitionConfig::AllTypes;
        let table = TestTableBuilder::new()
            .log_state(LogState::commits(2))
